...
def train_lymphocyte(output_file, input_folder):
    ...
    xae_learning_rate = 0.0002
    fcn_learning_rate = 0.0002
    ...
    for layer_i in range(len(xae_layers)):
        for epoch_i in range(n_xae_epochs): 
            ...
            if (epoch_i+1) % n_xae_display_per_epochs == 0:
                ...
                display_color_network(weights, filename='lymphocyte_weights.png')                             
                display_color_network(test_xs_0.transpose(), filename='lymphocyte_test_0.png')
                display_color_network(recon_0.transpose(), filename='lymphocyte_results_0.png')              
                display_color_network(test_xs_1.transpose(), filename='lymphocyte_test_1.png')
                display_color_network(recon_1.transpose(), filename='lymphocyte_results_1.png')              
    ...
    for epoch_i in range(n_fcn_epochs):
        ...
        if (epoch_i+1) % n_fcn_display_per_epochs == 0:
            ...
            test_xs, test_ys = lymphocyte_test_dataset[random.sample(range(lymphocyte_test_dataset.__len__()), n_fcn_batch_size)]
            ae_z = sess.run(xae['z'], feed_dict={xae['x']: test_xs})
            
            acc = sess.run(fcn['accuracy'], feed_dict={fcn['x']: ae_z, fcn['y_']: test_ys, fcn['keep_prob_input']: 1.0, fcn['keep_prob']: 1.0})
    ...
    plt.gcf().savefig('lymphocyte_rocplot.png')
    plt.clf()
...
# def lymphocyte_dataset_preparation(result_path, raw_img_path, label_img_path, patch_size):
#     for f in range(1, 101):
...
def nuclei_dataset_preparation(result_path, raw_img_path, label_img_path, patch_size):
    for f in range(1, 101):
        data = np.empty((0, patch_size*patch_size*3))
        label = np.empty((0, 2))  
        ...
        dataset = {
            'data': np.vstack((sample_ch0, sample_ch1, sample_ch2)).transpose(),
...
def lymphocyte_dataset_preparation(result_path, csv_file, raw_img_path, label_img_path, patch_size):
    cnt_pos = 0
    cnt_neg = 0
...
#     nuclei_dataset_preparation(
#         result_path='./lymphocyte/stage1', 
#         raw_img_path='./lymphocyte/normalized_data', 
#         label_img_path='./lymphocyte/manual_seg/center', 
#         patch_size=PATCH_SIZE)
     
    train_lymphocyte(output_file='lymphocyte_stage1.pickle', 
                     input_folder='./lymphocyte/stage1')
    
#     lymphocyte_dataset_preparation(
#         result_path='./lymphocyte/stage2', 
#         csv_file='MyExpt_IdentifyPrimaryObjects.csv', 
#         raw_img_path='./lymphocyte/normalized_data',
#         label_img_path='./lymphocyte/manual_seg/center', 
#         patch_size=PATCH_SIZE)
     
#     train_lymphocyte(
#         output_file='lymphocyte_stage2.pickle', 
#         input_folder='./lymphocyte/stage2')
